...
// Debug flag - set to 0 to disable network messages
#define DEBUG_NETWORK 0

// Base port definitions (will be offset based on command args)
#define BASE_PORT 23456
#define BASE_IPC_PORT 12347
#define BROADCAST_IP "172.20.15.255"
#define MAX_CLIENTS 10
#define UPDATE_INTERVAL 5000000 // 5 seconds in microseconds

// Global variables for port configuration
int port_offset = 0;
int game_port;
int ipc_port;

// Add this global variable:
int player_id = 1; // Default to player 1
...
void send_message_to_python_client(const char *message) {
    ...
    python_addr.sin_port = htons(ipc_port+1); // Use dynamic port + 1
    ...
    if (send(ipc_client_socket, message, strlen(message), 0) < 0) {
        ...
        if (DEBUG_NETWORK) printf("Message forwarded to Python client: %s\n", message);
        ...
        if (bytes > 0) {
            ack[bytes] = '\0';
            if (DEBUG_NETWORK) printf("Python client acknowledgment: %s\n", ack);
        }
    }
...
void broadcast_message(const char *message) {
    char player_message[1024];
    // Format: P<player_id>|<original message>
    snprintf(player_message, sizeof(player_message), "P%d|%s", player_id, message);

    struct sockaddr_in broadcast_addr;
    memset(&broadcast_addr, 0, sizeof(broadcast_addr));
    broadcast_addr.sin_family = AF_INET;
    broadcast_addr.sin_port = htons(game_port); // Use dynamic port
    broadcast_addr.sin_addr.s_addr = inet_addr(BROADCAST_IP);
    
    // Send the broadcast
    if (sendto(sockfd, player_message, strlen(player_message), 0, 
              (struct sockaddr*)&broadcast_addr, sizeof(broadcast_addr)) < 0) {
        printf("Failed to broadcast message: %d\n", errno);
    } else {
        if (DEBUG_NETWORK) printf("Broadcast message sent: %s\n", player_message);
    }
}
...
void *receive_messages(void *arg) {
    ...
    while (1)
    {
        ...
        if (DEBUG_NETWORK) printf("Received message from client: %s\n", buffer);
...
void *send_messages(void *arg) {
    ...
    serverAddr.sin_port = htons(game_port); // Use dynamic port

    while (1)
    {
        if (DEBUG_NETWORK) printf("Enter a message to broadcast: ");
        ...
        if (DEBUG_NETWORK) printf("Message broadcast to %s:%d\n", BROADCAST_IP, game_port);
    }
}
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (client_info[i].active && 
           (exclude_addr == NULL || 
            ...
            if (DEBUG_NETWORK) printf("Sending game message to client %d: %s\n", i, gameData->message);
        }
    }
}
...
void *handle_incoming_data(void *arg) {
    ...
    while (1) {
        ...
        if (recv_len > 0) {
            if (DEBUG_NETWORK) printf("Received game message from client: %s\n", received_data.message);
...
void *listen_for_ipc_commands(void *arg) {
    ...
    while (1) {
        ...
        while ((bytes_read = recv(client_sock, buffer, sizeof(buffer) - 1, 0)) > 0) {
            ...
            if (DEBUG_NETWORK) printf("Received IPC command: %s\n", buffer);
...
int main(int argc, char *argv[]) {
    // Parse command-line arguments for port offset
    if (argc > 1) {
        port_offset = atoi(argv[1]);
        if (DEBUG_NETWORK) printf("Using port offset: %d\n", port_offset);
    }

    // In main function, parse player ID too:
    if (argc > 2) {
        player_id = atoi(argv[2]);
        if (DEBUG_NETWORK) printf("Running as Player %d\n", player_id);
    }
        
    // Calculate actual ports
    game_port = BASE_PORT + port_offset;
    ipc_port = BASE_IPC_PORT + port_offset;
    
    if (DEBUG_NETWORK) printf("Game network using ports: Game UDP=%d, IPC TCP=%d\n", game_port, ipc_port);
    ...
    ipc_addr.sin_port = htons(ipc_port); // Use dynamic port
    ...
    server_addr.sin_port = htons(game_port); // Use dynamic port
    ...
    if (DEBUG_NETWORK) printf("All threads started successfully\n");
